...
**Shared memory (API + MCP):** use PostgreSQL (`docker compose up -d`, `pnpm db:migrate`, `KLM_STORE_BACKEND=postgres`). File store reloads from disk each operation but is **dev-only** for multi-process. Set fixed `KLM_PROJECT_ID` in MCP — not random per start.
